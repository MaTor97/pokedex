...
async function getPokemonEvolution(pokemonId) {
  ...
  console.log('Chaine évolution: ', evolutionChain);
...
async function getPokemonDetails(pokemonName) {
  try {
    ...
    const defaultImageUrl = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${pokemonId}.png`;
    const shinyImageUrl = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/shiny/${pokemonId}.png`;
      ...
      <main style="${backgroundStyle}">
        <div id='profile'>
          <div id='image-type'>
            <div id='assets'>
              <div id='name'>
                <p id='one'><strong>Pokemon</strong></p>
              </div>
              <div id='infos'>
                <p><strong>Name</strong> ${pokemonData.name.toUpperCase()} </p>
                <p><strong>Weight</strong> ${weight} kg</p>
                <p><strong>Height</strong> ${height} m</p>
                <strong>Shiny   </strong><input type='checkbox' id='shiny' />
              ...
              <div id='types'>
                ${pokemonType
                  .map(
                    type => `
                    <div style="background-color: ${pokemonTypes[type]}; color: white; padding: 2px 12px; border-radius: 15px; margin: 10px 0; border: 1px solid black;">
                      ${type.toUpperCase()}
                    </div>`
                  )
                  .join("")}
            ...
            <img id="pokemon-image" src="${defaultImageUrl}" alt="${pokemonName}" />
          </div>
        </div>
        <div id='data'>
        </div>
